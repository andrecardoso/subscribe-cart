...
describe('Cart', () => {
  ...
  describe('#total()', () => {
    it('should return sum of prices when all products are exempt', () => {
      const chocolate = new Product({ price: 9.99, isExempt: true })
      const bananas = new Product({ price: 1.99, isExempt: true })
      const pie = new Product({ price: 5.99, isExempt: true })
      ...
      assert.equal(9.99 + 1.99 + 5.99, cart.total())
    })

    it('should return sum of prices and taxes when all products are not imported', () => {
      const book = new Product({ price: 12.49, isExempt: true })
      const cd = new Product({ price: 14.99 })
      const chocolate = new Product({ price: 0.85, isExempt: true })
      ...
      assert.equal(42.32, cart.total())
    })

    it('should return sum of prices and taxes taxes when all products are imported', () => {
      const chocolate = new Product({
        price: 10.0,
        isExempt: true,
        isImported: true
      ...
      assert.equal(65.15, cart.total())
    })

    it('should return sum of prices and taxes when products are imported or exempt', () => {
      const perfume = new Product({
        price: 18.99,
        isImported: false,
        isExempt: false
